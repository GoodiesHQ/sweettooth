...
func (engine *SweetToothEngine) loopOnce() bool {
	...
	if BYPASS_SCHEDULE || schedule.Now() {
		engine.PackageJobs()
...
func (engine *SweetToothEngine) loop() {
	...
	for engine.isRunning() {
		...
		log.Info().Msg("client is not stopped")

		if err := engine.loopOnceRecoverable(); err != nil {
			// a panic is by definition unexpected behavior

			if err == ErrStop {
				log.Warn().Msg("stop panic received, engine has been stopped, the loop will not continue.")
				return
			} else {
