...
func loadKeys(cipher Cipher) error {
	keyMu.Lock()
	defer keyMu.Unlock()
	...
	secPem, err := DecipherFile(cipher, config.SecretKey())
	if err != nil {
		return err
	}
...
func saveKeys(cipher Cipher, pubKey ed25519.PublicKey, secKey ed25519.PrivateKey) error {
	keyMu.Lock()
	defer keyMu.Unlock()
	...
	err = EncipherFile(cipher, config.SecretKey(), secEncoded)
	if err != nil {
...
func generate(cipher Cipher) error {
...
func Bootstrap(cipher Cipher) error {
